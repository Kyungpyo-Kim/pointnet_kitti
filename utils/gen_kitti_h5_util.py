...
def room2blocks(data, label, num_point, block_size=1.0, stride=1.0,
    ...
    if not random_sample:
        num_block_x = int(np.ceil((ROI_X[1] - ROI_X[0] - block_size) / stride)) + 1
        num_block_y = int(np.ceil((ROI_Y[1] - ROI_Y[0] - block_size) / stride)) + 1
        for i in range(num_block_x):
            for j in range(num_block_y):
                xbeg_list.append(i*stride)
                ybeg_list.append(j*stride)
    else:
        num_block_x = int(np.ceil(ROI_X[1] / block_size))
        num_block_y = int(np.ceil(ROI_Y[1] / block_size))
        if sample_num is None:
            sample_num = num_block_x * num_block_y * sample_aug
        for _ in range(sample_num):
            xbeg = np.random.uniform(-block_size, ROI_X[1]) 
            ybeg = np.random.uniform(-block_size, ROI_Y[1]) 
...
def room2blocks_plus_normalized_xyz(data_label, x, y, z, num_point, block_size, stride,
                                random_sample, sample_num, sample_aug, is_train = True):
    """ room2block, with input filename and RGB preprocessing.
        for each block centralize XYZ, add normalized XYZ as 678 channels
    """
    data = data_label[:,0:3]
    ...
    max_local_x = data[:,0].max()
    max_local_y = data[:,1].max()
    max_local_z = data[:,2].max()

    min_local_x = data[:,0].min()
    ...
    if len(data_batch) >= 1:
        ...
        for b in range(data_batch.shape[0]):
            new_data_batch[b, :, 3] = (data_batch[b, :, 0] - min_local_x) / (max_local_x - min_local_x)
            new_data_batch[b, :, 4] = (data_batch[b, :, 1] - min_local_y) / (max_local_y - min_local_y)
            new_data_batch[b, :, 5] = (data_batch[b, :, 2] - min_local_z) / (max_local_z - min_local_z)
